  ...
  ({ className, ...props }, ref) => {
    return (
      <div
        ref={ref}
        className={cn(
          "grid grid-cols-2 grid-rows-[50px_150px_50px_150px_50px] gap-4 gallery-grid-new",
          className
        )}
        {...props}
      />
  ...
  ({ className, transition, index, ...props }, ref) => {
    return (
      <motion.div
        ...
        className={`relative overflow-hidden rounded-xl shadow-xl ${areaClasses[index]} gallery-cell-new cell-${index} ${className || ""}`}
        {...props}
      />
    )
  }
