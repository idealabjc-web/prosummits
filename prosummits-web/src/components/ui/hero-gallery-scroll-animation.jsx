...
const ContainerScroll = ({
  ...
  // Track scroll progress strictly during the sticky phase (start start to end end) for perfect timeline sync!
  ...
  return (
    <ContainerScrollContext.Provider value={{ scrollYProgress }}>
      <div
        ref={scrollRef}
        className={cn("relative min-h-screen w-full scroll-container-main", className)}
        {...props}
      >
        {children}
      </div>
...
const BentoGrid = React.forwardRef(({ variant, className, ...props }, ref) => {
  return (
    <div
      ref={ref}
      className={cn("scroll-bento-grid", bentoGridVariants({ variant }), className)}
      {...props}
    />
  )
})
...
const BentoCell = React.forwardRef(
  ({ className, style, ...props }, ref) => {
    ...
    return (
      <motion.div
        ref={ref}
        className={cn("scroll-bento-cell", className)}
...
const ContainerScale = React.forwardRef(
  ({ className, style, ...props }, ref) => {
    ...
    // Verbatim transforms exactly as specified in the original design component!
    const opacity = useTransform(scrollYProgress, [0, 0.5], [1, 0])
    const scale = useTransform(scrollYProgress, [0, 0.5], [1, 0])

    // Always absolute because BentoGrid is sticky and handles viewport lock perfectly!
    const position = "absolute"

    return (
      <motion.div
        ref={ref}
        className={cn("left-1/2 top-1/2 size-fit scroll-scale-content", className)}
        style={{
          translate: "-50% -50%",
          scale,
          position,
